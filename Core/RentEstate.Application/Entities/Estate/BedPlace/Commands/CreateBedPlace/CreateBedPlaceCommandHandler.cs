...
namespace RentEstate.Application.Entities.Estate.BedPlace.Commands.CreateBedPlace
{
    public class CreateBedPlaceCommandHandler : IRequestHandler<CreateBedPlaceCommand, Guid>
    {
        private readonly IEstateDbContext _dbContext;

        public CreateBedPlaceCommandHandler(IEstateDbContext dbContext) {
            _dbContext = dbContext;
        }

        public async Task<Guid> Handle(CreateBedPlaceCommand request, CancellationToken cancellationToken)
        {
            var bedPlace = new Domain.BedPlace
            {
                ...
                Floor = request.Floor,
                FloorsInHouse = request.FloorsIntHouse,
            ...
            await _dbContext.BedPlaces.AddAsync(bedPlace, cancellationToken);
            await _dbContext.SaveChangesAsync(cancellationToken);

            return bedPlace.EstateId;
        }
    }
}
